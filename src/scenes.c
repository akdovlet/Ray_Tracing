...
/*   Updated: 2025/01/23 15:27:15 by akdovlet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_data	cube_scene(t_img *img, t_mlx *mlx)
{
	...
	cube = cube_default();
	set_transform(&cube, multiply_matrix(rotate_x(radians(30)),
										multiply_matrix(translate(0, -1, -0.5), scale(0.5, 0.5, 0.5))));
	// cube.matter.pattern = radial_pattern(color_new(1, 0, 0), color_new(0, 0, 1));
