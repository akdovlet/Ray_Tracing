...
/*   Updated: 2025/02/02 17:25:51 by akdovlet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple	blinn_phong(t_light light, t_shape *shape, t_comps *c, bool shadowed)
{
	...
	{
		e_color = pattern_at_shape(shape->matter.pattern, *shape, c->overz);
	...
	lightv = tuple_normalize(tuple_substract(light.position, c->overz));
	ambient = tuple_multiply(e_color, shape->matter.ambient);
	if (shadowed)
		return (ambient);
	light_dot_normal = fmax(tuple_dot(lightv, c->normalv), 0.0);
	halfway = tuple_normalize(tuple_add(lightv, tuple_normalize(c->eyev)));
	factor = pow(fmax(tuple_dot(c->normalv, halfway), 0.0), shape->matter.shininess);
	tmp = tuple_multiply(e_color, shape->matter.diffuse);
	diffuse = tuple_multiply(tmp, light_dot_normal);
	reflectv = reflect(tuple_negate(lightv), c->normalv);
	reflect_dot_eye = fmax(tuple_dot(reflectv, c->eyev), 0.0);
