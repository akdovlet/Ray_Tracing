...
{
    conf->root->child[conf->pos.x][conf->pos.y] = &conf->root->zbuffer.buffer[conf->z_buffer_offset];
}

static void assign_root(t_conf* conf)
{
    conf->root->child[conf->pos.x][conf->pos.y]->root = conf->root;
    if (conf->root)
        conf->root->child[conf->pos.x][conf->pos.y]->depth = conf->root->depth - 1;
}
