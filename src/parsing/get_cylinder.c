...
/*   Updated: 2025/04/18 17:32:41 by akdovlet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_matrix	apply_transform(t_shape *cyl)
{
	t_matrix	mat;

	mat = multiply_matrix(t_translate(cyl->coordinates),
			rotate_axis(cyl->normal));
	mat = multiply_matrix(mat, scale(cyl->radius, cyl->radius, cyl->radius));
	return (mat);
}
...
int	cylinder(char *str, t_parse *parse, int line)
{
	...
	if (is_valid(str[i]))
	{
		...
		return (ft_dprintf(2, "Error: line %d: bad syntax: `%s'\n", line, str),
			1);
	set_transform(&cyl, apply_transform(&cyl));
